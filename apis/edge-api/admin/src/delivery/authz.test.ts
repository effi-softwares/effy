...
describe("delivery-management authz (from the admin.staff record, R6)", () => {
  ...
  it("canManageDelivery requires active AND admin/manager (mutate)", async () => {
    query.mockResolvedValue({ rows: [{ ok: true }] });
    expect(await canManageDelivery("sub-1")).toBe(true);
    expect(sql()).toContain("s.status = 'active'");
  ...
  it("denies when no row (absent = deny, fail-closed)", async () => {
    ...
    expect(await canManageDelivery("sub-1")).toBe(false);
  });
});
