// PATCH /admin/v1/catalog/attributes/{id} — edit an attribute definition. Mutate access.
...
import { updateAttribute } from "../catalog/service";
...
export const handler = async (
  ...
  try {
    return json(200, toAttributeDTO(await updateAttribute(event.pathParameters?.id ?? "", parsed.value, g.sub)), scope);
  } catch (err) {
