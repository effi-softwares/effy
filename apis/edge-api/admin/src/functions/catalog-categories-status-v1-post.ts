// POST /admin/v1/catalog/categories/{id}/status — retire/activate; retiring one with active products → 409 (FR-006). Mutate access.
...
import { changeCategoryStatus } from "../catalog/service";
...
export const handler = async (
  ...
  try {
    return json(200, toCategoryDTO(await changeCategoryStatus(event.pathParameters?.id ?? "", parsed.value.status, g.sub)), scope);
  } catch (err) {
