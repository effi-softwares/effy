// POST /admin/v1/catalog/product-types/{id}/attributes — assign an attribute. Mutate access.
...
import { assignAttribute } from "../catalog/service";
...
export const handler = async (
  ...
  try {
    return json(200, toProductTypeDTO(await assignAttribute(event.pathParameters?.id ?? "", parsed.value, g.sub)), scope);
  } catch (err) {
