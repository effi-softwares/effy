// POST /admin/v1/delivery-offerings — define a rate for an (origin zone → destination zone, method)
// leg (US4/FR-015). Mutate access. A duplicate leg-method → 409 (UNIQUE, 23505). Audits
// delivery_offering.create.
import type { APIGatewayProxyStructuredResultV2, Context } from "aws-lambda";

import type { AuthedEvent } from "@effy/edge-shared";
import { json, parseJsonBody, preamble, problem, ProblemType } from "@effy/edge-shared";

import { guard, mapDeliveryError, toOfferingDTO } from "../delivery/handler-support";
import { createOffering } from "../delivery/service";

export const handler = async (
  event: AuthedEvent,
  context: Context,
): Promise<APIGatewayProxyStructuredResultV2> => {
  const scope = preamble(event, context);
  const g = await guard(event, scope, "mutate");
  if ("deny" in g) return g.deny;

  const parsed = parseJsonBody<Record<string, unknown>>(event.body);
  if (!parsed.value) {
    return problem(400, ProblemType.ValidationFailed, "Validation failed",
      "a JSON body is required", scope, parsed.errors);
  }

  try {
    const offering = await createOffering(parsed.value, g.sub);
    return json(201, toOfferingDTO(offering), scope);
  } catch (err) {
    return mapDeliveryError(err, scope);
  }
};
