// POST /admin/v1/feedback/{referenceCode}/status — change triage status (046 US2). Any active staff.
...
import { changeStatus, detail } from "../feedback/service"
...
export const handler = async (
  ...
  const body = parseJsonBody<{ status?: unknown }>(event.body)
  ...
  try {
    await changeStatus(referenceCode, body.value.status)
