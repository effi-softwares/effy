...
describe.skipIf(!RUN)("shop_ops triggers — real PostgreSQL, real migrations", () => {
  ...
  async function paidOrder(number: string): Promise<{ orderId: string; fulfillmentId: string }> {
    ...
    const p = await pool.query<{ id: string }>(
      `INSERT INTO public.product (shop_id, name, slug, price_amount, status)
       VALUES ($1,'Milk','milk-' || $2,10,'active') RETURNING id`,
      [SHOP, number],
    );
    await pool.query(
  ...
  it("a committed refund marks exactly one bucket and pokes the shop", async () => {
    ...
    await pool.query(
      `INSERT INTO public.refund (order_id, kind, amount, reason, idempotency_key, actor_kind)
       VALUES ($1,'item',10,'item_not_supplied','k-1','back_office')`,
      [orderId],
    );
    await settle();
  ...
  it("⚠ a ROLLED-BACK refund leaves no dirty mark and sends no poke", async () => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO public.refund (order_id, kind, amount, reason, idempotency_key, actor_kind)
         VALUES ($1,'item',10,'item_not_supplied','k-2','back_office')`,
        [orderId],
      );
  ...
  it("many changes in ONE transaction collapse to a single poke", async () => {
    ...
    try {
      ...
      for (let i = 1; i <= 5; i++) {
        await client.query(
          `INSERT INTO public.fulfillment_item (shop_fulfillment_id, order_item_id, gathered_quantity)
           VALUES ($1,$2,$3)
           ON CONFLICT (shop_fulfillment_id, order_item_id) DO UPDATE SET gathered_quantity = EXCLUDED.gathered_quantity`,
          [fulfillmentId, item.rows[0]!.id, i],
        );
      }
  ...
  it("the first mark's marked_at survives later marks, so backlog age is the real lag", async () => {
    ...
    await pool.query(
      `INSERT INTO public.refund (order_id, kind, amount, reason, idempotency_key, actor_kind)
       VALUES ($1,'item',10,'item_not_supplied','k-4','back_office')`,
      [orderId],
    );
