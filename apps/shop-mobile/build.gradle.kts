...
val requiredKeys = listOf(
    "COGNITO_USER_POOL_ID",   // the SHOP pool — a NAME, not a key
    "COGNITO_APP_CLIENT_ID",  // the SHOP MOBILE client id — a NAME, not a key; NO client secret
    "COGNITO_REGION",
    "SHOP_API_BASE_URL",      // edge-api/shop — the ONLY backend this app calls
)
...
if (missingKeys.isNotEmpty()) {
    throw GradleException(
        """
        |
        |╭─ shop-mobile: missing required build configuration ────────────────────────────────────
        |│ Missing: ${missingKeys.joinToString(", ")}
        |│
        |│ Set each in ${secretsFile.path} (git-ignored) or as an environment variable.
        |│ Template: apps/shop-mobile/secrets.properties.example
        |│ Values:   make output ENV=dev   ·   SSM /effy/<env>/auth/shop/* and /effy/<env>/edge/api_endpoint
        |╰────────────────────────────────────────────────────────────────────────────────────────────
        """.trimMargin()
    )
}
