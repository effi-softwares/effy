...
## §032 — Same-day declaration & product weight

**Status**: implemented and machine-verified; operator walks outstanding.

| Capability | shop-web | shop-mobile |
|---|---|---|
| State whether this shop offers same-day | ✅ `/delivery` (manager only) | ⚠ **outstanding by design** |
| Choose the areas it can reach, **by name** | ✅ locality search, not a postcode box | ⚠ outstanding |
| See what a chosen area actually covers | ✅ shared `PostcodeCoverageNotice` | ⚠ outstanding |
| See what is live vs waiting for approval | ✅ both, as separate facts | ⚠ outstanding |
| Read why a request was declined | ✅ with the admin's reason | ⚠ outstanding |
| Record a product's real weight | ✅ create + edit, flagged measured/assumed | ⚠ outstanding |
| **Set a delivery fee** | ⛔ **impossible, by design** | ⛔ impossible |

⚠ **The mobile column is outstanding BY DESIGN, not by omission.** `shop-mobile` gains no delivery
screen in this slice. Building it is its own slice, exactly as 007's mobile column was.

### ⚠ A shop cannot see or set a delivery fee, and that is structural

FR-008 is enforced by **route topology**: `apis/edge-api/shop` defines no pricing path at any verb, so
a shop token cannot reach one even by hand. There is no button to hide and no check to forget.

⚠ **SC-004 is verified by calling the ADMIN pricing route with a real shop token and confirming the
401** — not by observing that this console has no button.

### ⚠ A declaration is a proposal, not a switch

Saving changes **nothing** for any shopper (FR-017). An admin must approve it, and anything already
approved keeps working until they do (FR-018) — the console shows both as separate facts, because a
single "current" line would make a shop believe either that a pending edit was live or that an
approved one had been lost.

⚠ **`revoked` and `superseded` read differently**: an admin withdrawing same-day is not the same event
as this shop's own update going live.

### ⚠ An area is a postcode

Choosing **"Alfredton"** commits the shop to all **twenty** Ballarat localities, because serviceability
is postcode-decided everywhere on this platform. The disclosure says so **before** the shop confirms,
and it is fed real coverage from the server — 031 shipped two disclosures hardcoded to zero, so
neither would ever have rendered.

### ⚠ Two location refusals, not one

- **No postcode on the shop** → it cannot declare, and is told why.
- **A postcode with no known coordinates** → also refused, *separately*. Such a shop would otherwise
  pass the obvious check and then produce a **blank distance for every area** at approval time, so the
  admin decides blind — the exact failure the approval screen exists to prevent. This is 031's live
  3001 case (a real postcode no locality names) reaching a second surface.

### Known limits

- ⚠ **24 of 38 products carry an ASSUMED weight.** Delivery is priced from it either way; the flag is
  what makes the gap visible rather than absorbed. An operator can correct any of them.
- **No email or push** when a decision lands. In-console status only.
- ⚠ **No cold-path metric exists** for any of this — no service on this platform emits one, and no
  Prometheus is provisioned to read it if it did. **Carry-forward.**
