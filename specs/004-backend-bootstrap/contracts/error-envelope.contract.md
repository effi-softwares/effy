# Contract: Error Envelope (RFC 9457 Problem Details)

**Binding on**: every failure response from `core-api` and `edge-api`, all versions.
**Ships as**: `docs/api/error-envelope.md` (single source of truth) + conformance tests
in each service. **Decision**: research.md A6.
