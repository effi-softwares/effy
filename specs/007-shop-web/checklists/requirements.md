...
|---|----------|--------|--------------|
| Q1 | Store role model and origin of role assignment | Two roles — `store manager` / `store staff` — as **role groups on the store identity pool**, mirroring the back-office pattern. Roles reconciled into the platform record; status platform-owned. | **Expands**: the store identity pool must be amended to carry role groups (an infrastructure change), and constitution Principle IV must be reconciled at `/plan` (it names role groups only on the admin pool). Recorded as an explicit assumption, **not** left for code to resolve. |
| Q2 | Store entity and staff-to-store scoping | **Yes** — a minimal store record (identity, code, name, active flag) in the **customer-operational** data area, with each operator assigned to at most one store. Authorization becomes **role AND status AND store scope**. | **Expands**: the platform's first customer-operational records; store records are operator-seeded, no management interface. Bounded by FR-025, which forbids any operational attribute on the store record. |
