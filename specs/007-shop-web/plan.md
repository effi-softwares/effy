...
Bootstrap the platform's **second web surface** — the shop operator console (`apps/shop-web`) —
on the same stack as the back-office console, authenticating against the **shop** Cognito pool,
with a shop role model the platform does not have yet.

The technical approach, in one breath: **amend the constitution** (a second pool gains RBAC
groups → v1.5.0), **give the shop pool two groups** (`shop_manager` / `shop_staff`) and add the
new dev origin to the shared gateway's CORS in one Terraform apply, **create the platform's first
`public`-schema tables** (`shop`, `shop_staff`, `shop_role`, `shop_staff_role`) via the 003
forward-only workflow, **extend the existing `shop` edge service** with a record-backed
`/shop/v1/me` and a DB-gated `/shop/v1/manager-ping` (gate = role AND status AND shop scope),
...
copies it, then **build `shop-web`** on that foundation and record the shop audience's
...
**Testing**: Vitest 3 + jsdom + Testing Library (`shop-web`, `web-kit`, `back-office`); Vitest node (`@effy/edge-shop`). Cross-pool isolation and live RBAC are **operator-run** (`curl` + real tokens), not unit-testable — see research R9.
...
**Constraints**: No new backend npm dependency. No hosted deploy (FR-001). No product shop-operations features (FR-025). No shop-management interface (FR-019). Back-office's 20 tests must stay green through the extraction. The local origin must be an *approved* origin — a Terraform change, not a code change.
...
- **What changes**: Principle IV's RBAC sentence generalizes from "the admin pool defines groups"
  to "pools MAY define groups", enumerating admin (`admin`/`manager`/`csa`) and shop
  (`shop_manager`/`shop_staff`), with customer and driver defining none, and restating that the
...
│   ├── shop-me.contract.md
│   ├── shop-manager-ping.contract.md
│   ├── shop-schema.contract.md
...
CLAUDE.md                            # reconciled: shop-web → shop-web; Auth RBAC sentence

infra/envs/dev/
├── auth-shop.tf                     # + groups = [shop_manager, shop_staff]
└── edge-gateway.tf                  # + "http://localhost:5174" in cors allow_origins

db/migrations/
└── <ts>_shop_staff_rbac.sql        # NEW — public.shop / shop_staff / shop_role / shop_staff_role

apis/edge-api/shop/
├── serverless.yml                   # + shopMeV1, shopManagerPingV1 (+3 alarms)
└── src/
    ├── functions/
    │   ├── shop-me-v1-get.ts               # NEW
    │   ├── shop-manager-ping-v1-get.ts     # NEW
    │   ├── shop-ping-v1-get.ts             # unchanged (004 token echo)
...
└── shop-web/                        # NEW — @effy/shop-web, vite :5174
    ...
    └── src/
        ...
        ├── components/layout/nav.ts                 # shop nav config (role-gated)
        └── features/
            ├── auth/{repo,queries,model,SignInScreen}.ts(x)
            └── shop-identity/{repo,queries,ProvingScreen,ManagerOnlyScreen}.ts(x)

docs/audiences/
└── shop-capabilities.md            # NEW — the parity register (FR-023a), single source; linked
...
with `apps/shop-mobile` — the two halves of the shop audience that the parity register binds. The
backend keeps its deployed name (`apis/edge-api/shop`, routes `/shop/v1/...`). See research R1
for the full reasoning; the rule is stated once below and not re-derived elsewhere.

### Terminology (stated once)

**It is `shop` everywhere** — the surfaces, the pool, the gateway authorizer, the backend service,
its route paths, its tables, its roles, and the audience in prose.

> **Revised by 008-shop-naming-unification.** As shipped, 007 kept a split: the backend service and
> its paths were named `store` while everything client-facing was `shop`. 008 retired that split;
> this plan's artifacts have been reconciled to the single name. See
> [008 research R1](../008-shop-naming-unification/research.md) for why 007's cost estimate was wrong.
...
| 3 | **Data** | Migration (4 tables). **No seed** — shops are created by back-office shop management (next slice), never by hand | ✋ commit, then `make db-up ENV=dev` |
| 4 | **Backend** | `shop` service: `staff/` domain, `/shop/v1/me`, `/shop/v1/manager-ping`, alarms, unit tests | ✋ `make edge-deploy SERVICE=shop ENV=dev` |
...
| 7 | **Parity + docs** | `docs/audiences/shop-capabilities.md`; READMEs; contracts | no |
...
| **A shop entity in the `public` schema** — the platform's first customer-operational table, in a slice whose FR-025 forbids product features | FR-019/FR-021: shop-scoped authorization is meaningless without a shop to scope to. The operator chose this at Q2. Bounded hard by FR-025: identity, code, name, active flag — no address, hours, capacity, zones, or inventory, and no management interface. | *Staff-only, no shop entity* was the alternative at Q2 — rejected by the operator; it would have made "shop scope" a phrase with nothing behind it. *Put the tables in `admin`*: conflates two audiences' identity systems inside the schema designated for back-office accounts + audit. |
| **Nullable `shop_staff.email`** | The shop pool uses email-as-username, so a Cognito **access token may carry no `email` claim** (research R6). Rather than guess Cognito's semantics inside a migration, email is operator-authoritative at provisioning and opportunistically refreshed from the token. | *Assume `username` is the email* — which is what 005 does, and which likely shops a UUID in `admin.staff.email` today (flagged as a 005 defect in R6). *Call `AdminGetUser` on first contact*: adds IAM and a network hop to the auth path for a value provisioning already knows. |
| **Restructuring `shop/src/` from flat to nested domains** | The service gains a second domain (staff alongside status). The admin service already uses the nested form; matching it makes the two structurally identical (Principle VI). | *Nest only staff and leave status flat*: two layouts in one service, and the next reader has to ask why. A mechanical move is cheaper than a permanent inconsistency. |
...
- **A defect in 005 was surfaced, not fixed here** (research R6): `/admin/v1/me` resolves email as
  ...
  inherit the pattern.
- **SC-004 (cross-pool isolation) and SC-005a (shop-scope denial) are operator-verified**, not
