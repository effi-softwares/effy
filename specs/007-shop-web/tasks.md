...
- [X] T002 Reconcile `CLAUDE.md`: platform-shape line `store-web` → `shop-web`; Auth section's "the admin pool defines RBAC groups" sentence updated to match constitution v1.5.0.
...
- [X] T007 [P] Add `groups = [{ name = "store_manager", ... }, { name = "store_staff", ... }]` to the `shop_pool` module block in `infra/envs/dev/auth-shop.tf`, with descriptions from [data-model.md](./data-model.md) § `public.store_role`.
...
- [X] T010 Create the migration via `make db-new name=store_staff_rbac`, then author `db/migrations/<ts>_store_staff_rbac.sql`: `public.store`, `public.store_staff`, `public.store_role` (seeded with both roles), `public.store_staff_role` — exact columns, constraints, FK actions and indexes per [data-model.md](./data-model.md) and [store-schema.contract.md](./contracts/store-schema.contract.md). Goose `Up`/`Down` sections; `Down` drops in FK-safe order.
- [X] T011 [P] ~~Create `db/seeds/dev-store.sql`~~ — **REMOVED 2026-07-10.** Store rows are created by back-office store management (next slice), never by hand: no seed file, no command (FR-019, revised). The migration creates the empty table and nothing writes to it here.
...
- [X] T026 [P] [US1] Create `apps/shop-web/src/features/auth/model.ts` — `Identity { subject, email, roles: StoreRole[] }`, `Session` union (`signed-in` | `signed-out`), `isStoreManager(roles)`.
- [X] T027 [P] [US1] Create `apps/shop-web/src/features/auth/repo.ts` — `loadSession()` reads the Amplify session: `subject` from the access token, `email` from the ID token, roles via `toStoreRoles(access.payload["cognito:groups"])`. Re-export `startSignIn`/`submitOtp`/`signOutUser` from `@effy/web-kit`.
...
- [X] T032 [P] [US1] Create `apps/shop-web/src/components/layout/nav.ts` — `NAV = [{ Dashboard, "/" }, { Management, "/manager", requiredRole: "store_manager" }]` and `visibleNav(roles)`.
...
- [ ] T034 ✋ [US1] Provision three shop-pool accounts in Cognito (identity only, no database): `sam.manager@effy.test` → `store_manager`, `ravi.staff@effy.test` → `store_staff`, `nobody@effy.test` → no group. Commands in [quickstart.md](./quickstart.md) §3. Then `make shop-dev`, sign in as each (this creates their `store_staff` rows via the JIT upsert), and confirm zero password prompts plus a clean shell in light **and** dark. Sam's proving screen must show the **"no store assigned"** state — the expected result (SC-002, SC-003, SC-013).
...
- [X] T035 [P] [US2] Create `apis/edge-api/store/src/staff/types.ts` — `StoreRole`, `KNOWN_ROLES`, `StoreStaffStatus`, `StoreSummary`, `StoreStaffRecord`.
- [X] T036 [US2] Create `apis/edge-api/store/src/staff/repository.ts` — `upsertOnContact(sub, email, tokenRoles)` inside `withTransaction`: `INSERT … ON CONFLICT (cognito_sub) DO UPDATE SET email = COALESCE(EXCLUDED.email, store_staff.email), last_seen_at = now(), updated_at = now()`; delete role rows not in the claim; insert the claim's roles `ON CONFLICT DO NOTHING`; filter unknown group names **before** reconcile; read back joined to `public.store`. Explicit row→domain mapping; raw SQL as named constants. **`status` and `store_id` are never written here.**
- [X] T037 [US2] Create `apis/edge-api/store/src/staff/service.ts` — `recordAndLoad(sub, email, tokenRoles)`. No HTTP, no SQL.
- [X] T038 [US2] Create `apis/edge-api/store/src/functions/store-me-v1-get.ts` — `preamble` → `subject` or `401` → resolve email as `claim("email") ?? emailShaped(claim("username")) ?? null` (research R6) → `recordAndLoad` → `200` with `{subject,email,roles,status,store,lastSeenAt}`; catch → `unavailable` (`503`). Log `subject` only — **never** the email (Principle VII).
- [X] T039 [US2] Add `storeMeV1` to `apis/edge-api/store/serverless.yml` — `httpApi GET /store/v1/me`, authorizer by id `${ssm:/effy/${sls:stage}/edge/authorizer/shop_id}`; alarms `StoreMeV1ErrorsAlarm` (Errors>0) and `StoreMeV1DurationP95Alarm` (p95 > 5000ms).
...
- [X] T042 [P] [US2] Create `apps/shop-web/src/features/store-identity/repo.ts` (`loadMe()` → `api.get<StoreStaffRecordDTO>("/store/v1/me")`, mapped to `StoreStaffRecord` via `toStoreRoles`) and `queries.ts` (`meQuery`).
- [X] T043 [US2] Create `apps/shop-web/src/features/store-identity/ProvingScreen.tsx` — renders subject/email/roles/**assigned store**; a **role-less** state; a **no-store-assigned** state (emits `shop_store_assignment_missing`); `unavailable` → degraded + Retry; `unauthenticated` → session recovery. Never raw `detail`. Render it inside the dashboard.
- [X] T044 [P] [US2] Add `apps/shop-web/src/features/store-identity/ProvingScreen.test.tsx` — pending, populated, role-less, no-store, degraded+retry, expired-session states.
...
- [X] T046 [US3] Add `authorizeStoreManager(sub)` to `apis/edge-api/store/src/staff/repository.ts` — the single three-term `EXISTS` predicate (role AND `status='active'` AND `JOIN public.store` with `st.is_active`) from [data-model.md](./data-model.md). Named SQL constant, raw SQL.
- [X] T047 [US3] Add `isActiveStoreManager(sub)` to `apis/edge-api/store/src/staff/service.ts`.
...
- [X] T049 [US3] Add `storeManagerPingV1` to `apis/edge-api/store/serverless.yml` — `httpApi GET /store/v1/manager-ping`, shop authorizer by id; `StoreManagerPingV1ErrorsAlarm`.
- [X] T050 [P] [US3] Add `apis/edge-api/store/src/functions/store-manager-ping.test.ts` covering contract tests 1–6: active manager at an active store → `200`; `store_staff` → `403`; disabled manager → `403`; manager with `store_id IS NULL` → `403`; manager at an inactive store → `403`; repository throws → `503` not `200`. Assert the `403` body discloses no term.

### Client — role-aware interface

- [X] T051 [P] [US3] Create `apps/shop-web/src/features/store-identity/ManagerOnlyScreen.tsx` + its `managerPingQuery` in `queries.ts` and `loadManagerPing()` in `repo.ts` — checking / confirmed / access-denied (`forbidden`) / degraded+Retry states; emits `shop_manager_area_access_denied` on denial.
...
- [X] T053 [P] [US3] Add `apps/shop-web/src/features/store-identity/ManagerOnlyScreen.test.tsx` and extend `nav.test.ts`: Management is hidden for `store_staff` and role-less; shown for `store_manager`; the denied state renders and fires the telemetry event exactly once.
- [ ] T054 ✋ [US3] **SC-005** — redeploy (`make edge-deploy SERVICE=store ENV=dev`), then `make shop-verify-gate MANAGER_TOKEN=… STAFF_TOKEN=… NOBODY_TOKEN=… ENV=dev`. Asserts `/store/v1/me` admits everyone while `/store/v1/manager-ping` refuses `store_staff` and role-less callers by the **backend**, and that the `403` body names no failing term. Every request bypasses the interface. (The manager-**served** case is SC-005b, deferred — no store can exist yet.)
...
- [X] T055 [P] [US4] ~~Add `shop-seed-store` to `Makefile`~~ — **REMOVED 2026-07-10.** Creating a store is back-office store management's job (next slice); a seed command would be dead the day it lands, and would let a store row exist that the product never created.
- [X] T056 [P] [US4] ~~Add `shop-provision-staff` to `Makefile`~~ — **REMOVED 2026-07-10.** Assigning a store, setting `status`, and setting `email` are all **store-staff management**, and belong in the back-office console alongside store creation. `store_id` and `status` therefore have no write path in this slice — which is exactly what makes the unassigned-manager denial (SC-005a) provable.
- [X] T057 [US4] Add ownership tests to `apis/edge-api/store/src/staff/repository.test.ts`: a token carrying `status` or `store_id`-shaped data **never** writes those columns; a null token email **never** overwrites a stored one (`COALESCE`); role reconcile leaves `status` and `store_id` untouched.
...
- [X] T059 [P] [US4] Add an audit-persistence test: when all roles are removed from the claim, the `store_staff` row **persists** with `roles: []` and grants nothing — the record is never deleted on role removal ([data-model.md](./data-model.md) § State transitions).
- [ ] T060 ✋ [US4] **SC-011 / SC-005a** — after all three accounts have signed in once, confirm exactly one `store_staff` row per account (none duplicated by repeated reloads), every `store_id` NULL, and `last_seen_at` advancing. `make shop-verify-gate …` (default `EXPECT_STORE=0`) must show the **`store_manager` refused for lack of a store** — the role alone grants nothing. **SC-005b and SC-012 are deferred** to the back-office store-management slice: disabling an operator and deactivating a store are management writes this slice ships no path for. Runbook: [quickstart.md](./quickstart.md) §6.
...
- [X] T063 [P] [US5] Add a "Surface parity" section to `apps/shop-mobile/README.md` linking to `docs/audiences/store-capabilities.md` as the binding baseline for this app.
...
- [X] T065 [P] [US5] Register the two new routes in the platform API docs: add `/store/v1/me` and `/store/v1/manager-ping` to `docs/api/path-assignment.md` § worked examples (or the store-service section) and to `docs/api/shared-gateway.md`'s route inventory.
...
- [X] T067 [P] **SC-008 secret & PII sweep**: `git grep -nE '(us-east-1_|ap-southeast-1_|AKIA|eyJ)' -- apps/shop-web packages/web-kit` returns nothing; `.env` is git-ignored and `.env.example` holds no real values; confirm no telemetry event carries the email, the OTP code, a token, or a store code — only `subject`.
