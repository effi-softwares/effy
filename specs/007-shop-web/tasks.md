...
- [X] T011 [P] Create `db/seeds/dev-store.sql` — an idempotent (`ON CONFLICT (code) DO NOTHING`) insert of one dev store.
...
- [ ] T034 ✋ [US1] Provision three shop-pool accounts and verify sign-in (SC-002, SC-013): `make shop-create-account EMAIL=sam.manager@effy.test ROLE=store_manager ENV=dev`, then `ravi.staff@effy.test ROLE=store_staff`, then `nobody@effy.test` (no ROLE — the role-less case). Run `make shop-dev` and sign in as Sam: confirm zero password prompts and that the shell renders in light **and** dark. Runbook: [quickstart.md](./quickstart.md) §3–§4.
...
- [X] T055 [P] [US4] Add `shop-seed-store` to `Makefile` — `make shop-seed-store CODE=… NAME=… ENV=dev`, composing the DSN via `infra/scripts/db-dsn.sh` (no credential in shell history, exactly as `db-up` does); idempotent on `code`.
- [X] T056 [P] [US4] Add `shop-provision-staff` to `Makefile` — `make shop-provision-staff EMAIL=… STORE=<code> [STATUS=active] ENV=dev`, setting `email`, `store_id`, `status` on `public.store_staff`. Runs **after** the operator's first sign-in (the row is created by the JIT upsert). Fails clearly when the email or store code is unknown.
...
- [ ] T060 ✋ [US4] **SC-011 / SC-012 / SC-005a** — seed and provision (`make shop-seed-store`, `make shop-provision-staff`), then flip each **platform-owned** term in turn (`status='disabled'`, `store_id=NULL`, `store.is_active=false`) and re-run `make shop-verify-gate` after each: the manager's `200` must become `403` **while their token stays valid**. Restore each. Then confirm exactly one `store_staff` row after several reloads, with `last_seen_at` advancing. Runbook: [quickstart.md](./quickstart.md) §6.
