...
A shop operator opens their catalog page and starts **Add product**. On web this opens a focused **drawer/dialog**; on mobile a **bottom-sheet drawer**. The flow is a clear, **multi-step form** with obvious separation of concerns: first choose the **product type**, then fill the **universal mandatory basics** (name, category, price, short description), then add the **primary image** on its own dedicated step (drag-and-drop, click-to-browse, or paste), then the **type-specific attributes** (mandatory ones required, optional ones clearly marked skippable), then **review & publish**. Work-in-progress is preserved as a **local draft** (device-local, not saved to the platform) so an interrupted operator can resume; the draft is discarded on submit or explicit discard.
...
- **FR-010b**: The primary image MUST be captured on a **dedicated step** of the creation flow — **step 3**, immediately after Basics and before the type-specific attributes step — presented as a modern **image dropzone** that accepts the image by **drag-and-drop**, **click-to-browse**, and **clipboard paste**, with a live **preview**, a replace/remove control, and client-side **type/size validation** mirroring the platform's accepted formats (JPEG / PNG / WebP, ≤ 10 MB). Publication remains blocked until a primary image is present (FR-010). The image is captured here; it is uploaded on publish (create-then-attach, R9).
