...
- [ ] T002 **⚠ OPERATOR — BLOCKING** Verify the chosen locality dataset is redistributable: read its actual licence, confirm it permits committing to this repo, and confirm it carries locality + state + postcode per row (research [R1](./research.md)). ⚠ If it does not, stop — the fallback is ABS SAL joined to Australia Post postcodes and it changes T003
- [ ] T003 Add the dataset to `db/reference/au-localities.csv` (~16–18k rows, postcode as **text**)
- [ ] T004 [P] Add `db/reference/README.md` recording source URL, licence, retrieval date, and row count at time of retrieval
