...
- [ ] T001 [P] Commit the G-NAF-derived place dataset to `db/reference/au-localities.csv` (columns:
...
- [ ] T002 [P] Create `packages/shared-types/src/delivery.ts` with customer-facing DTOs + enums:
...
- [ ] T003 [P] Create `packages/shared-types/src/delivery-admin.ts` with back-office DTOs + enums: zone,
  ring, fee-plan (+ring-price, weight-band), same-day exception, collection-run, settings, health-flag
  (contracts/delivery-admin-api)
- [ ] T004 [P] Export the two new modules from `packages/shared-types/src/index.ts`
- [ ] T005 [P] Scaffold the hot-path package dir `apis/core-api/internal/platform/delivery/` (empty
  `engine.go`, `zone.go`, `plan.go` with package decl + doc comments)
- [ ] T006 [P] Scaffold the cold-path domain dir `apis/edge-api/admin/src/delivery/` and the back-office
...
- [ ] T007 Author the single forward-only migration `db/migrations/<ts>_delivery_shipping_engine.sql`
...
- [ ] T008 Add the loader `apis/core-api/cmd/load-localities/main.go` + `internal/platform/delivery/localityload.go`
...
- [ ] T009 [P] Generate the Kotlin customer DTOs from `@effy/shared-types` into
  ...
  covers them
- [ ] T010 [P] Write the Go↔Kotlin wire-contract test skeletons — `apis/core-api/internal/features/checkout/wire_contract_test.go`
...
- [ ] T011 [P] Add a config-contract test in `apis/edge-api/admin/src/delivery/config.contract.test.ts`
  reading the real `serverless.yml` (the 035/038 guard) — routes + any new env keys the delivery domain declares
- [ ] T012 ⚠ **OPERATOR**: `make db-up ENV=dev` (after committing T007 — 003 commit-guard), then
...
- [ ] T013 [P] [US1] Serviceability + locality read tests in
...
- [ ] T014 [US1] Implement `postcode → active zone` predicate as a shared SQL const/func in
...
- [ ] T015 [US1] Implement `GET /v1/storefront/serviceability` + `GET /v1/storefront/localities` in
...
- [ ] T016 [P] [US1] Admin zone repo + service tests in `apis/edge-api/admin/src/delivery/service.test.ts`
...
- [ ] T017 [US1] Implement the admin zone slice — `apis/edge-api/admin/src/delivery/{authz.ts,repository.ts,service.ts,types.ts}`
...
- [ ] T018 [P] [US1] Back-office zones console in `apps/back-office/src/features/delivery/zones/` — list +
...
- [ ] T019 [P] [US1] Pure engine table test in `apis/core-api/internal/platform/delivery/engine_test.go`
...
- [ ] T020 [US1] Implement the pure fee engine `apis/core-api/internal/platform/delivery/engine.go`
...
- [ ] T021 [US1] Implement active-plan load + coverage validation in
...
- [ ] T022 [P] [US1] Admin ring + fee-plan repo/service tests in the admin `service.test.ts`/`repository.container.test.ts`:
...
- [ ] T023 [US1] Implement the admin ring + fee-plan slice (`repository.ts`/`service.ts` extensions) +
...
- [ ] T024 [P] [US1] Haversine ring **suggestion** in `apis/edge-api/admin/src/delivery/suggest.ts`
...
- [ ] T025 [P] [US1] Back-office rings + fee-plan editor in `apps/back-office/src/features/delivery/{rings,plans}/`
...
- [ ] T026 [P] [US1] Product weight entry in `apis/edge-api/shop/src/products/` (+ test): `PATCH` accepts
  `weightGrams` and sets `weightIsAssumed=false`; list filter `assumedWeight:true` (FR-054/055)
- [ ] T027 [US1] Implement the **standard** delivery quote in `apis/core-api/internal/features/checkout/`
  ...
  contracts §3; FR-029)
- [ ] T028 [US1] Capture + honour the quote in checkout intent/finalize: write `order.delivery_quote`
...
- [ ] T030 [P] [US1] customer-web: show serviceability in the delivery affordance and the standard fee in
...
- [ ] T032 [US1] Hot-path metrics: a serviceability counter labelled `serviced` only, and a quote counter by
...
- [ ] T033 [P] [US2] Same-day timing tests in `apis/core-api/internal/platform/delivery/zone_test.go`:
...
- [ ] T034 [US2] Implement collection-schedule + settings admin in the admin delivery slice + handlers
...
- [ ] T035 [US2] Implement zone `sameday_eligible` toggle via `PATCH .../zones/:id` in the admin slice
  (FR-037) + audit
- [ ] T036 [US2] Implement same-day eligibility + derived-cutoff resolution in
  `apis/core-api/internal/platform/delivery/zone.go` (`SameDayOfferedForZone(now, runs, buffer, eligible)`)
- [ ] T037 [US2] Extend the checkout quote to add the `same_day` option per package when eligible + before
...
- [ ] T038 [P] [US2] Back-office collection-schedule + settings + zone same-day-eligible controls in
...
- [ ] T040 [US2] Quote metric: same_day/standard/unserviced outcome + chosen method (extend T032; no PII)
...
- [ ] T041 [P] [US3] Exception-resolution + per-package tests: admin `service.test.ts` (upsert on/off per
...
- [ ] T042 [US3] Implement `shop_sameday_exception` admin CRUD in the admin delivery slice + handlers
  `apis/edge-api/admin/src/functions/sameday-exception-*.ts` + routes (mutate=admin/manager; audit)
- [ ] T043 [US3] Resolve same-day **per fulfilling shop** in the quote — extend `zone.go`/checkout to apply
  the shop's exception over the zone default (FR-044)
- [ ] T044 [P] [US3] Back-office same-day exceptions surface in `apps/back-office/src/features/delivery/sameday/`
  (per shop×zone list; no cards)
- [ ] T045 [US3] ⚠ Prove the **shop** service exposes no delivery-config route at all — neither same-day
...
- [ ] T046 [P] [US4] Plan-register + immutability tests: admin `service.test.ts` (list shows one active;
...
- [ ] T047 [US4] Back-office fee-plan **register** in `apps/back-office/src/features/delivery/plans/` — list
...
- [ ] T048 [US4] Confirm quoted-fee immutability end to end against a live plan switch (the capture from T028
...
- [ ] T049 [P] [US5] shop-web product weight UI in `apps/shop-web/src/features/catalog/` — weight field
  (measured on save), an "assumed weight" filter/report to surface products still needing a real weight
- [ ] T050 [US5] shop-web/mobile test: assumed→measured flips `weightIsAssumed` and the quote weight-slab
...
- [ ] T051 [P] Wire the **invariant alarm**: a served zone that fails to produce a standard fee (must never
  fire) — metric + Grafana alert (Principle VII; research R12)
- [ ] T052 [P] ⚠ No-PII sweep: assert no postcode/suburb/coordinate appears in logs, metric labels, or
  telemetry, and no distance/ring-name/shop-id in any customer DTO (SC-007; quickstart §8)
- [ ] T053 [P] Update the parity register
...
- [ ] T054 [P] customer-web bundle check within **174 KB** across all guest routes (`pnpm size`); mobile
...
- [ ] T056 Full machine sweep: `go test ./...` (engine + container-backed, `-short` split) · `pnpm -r
