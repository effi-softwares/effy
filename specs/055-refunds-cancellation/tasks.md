...
- [X] T078 **NEGATIVE PROOFS ✅ EXECUTED** on both skip conditions (collected, untracked). FR-030 stock return FR-030 stock return (**US1**'s last piece) in `refunds/stock.go` — ⚠ **only** for an item-derived refund on an uncollected portion of a tracked product ([research.md](./research.md) R8). A goodwill refund returns nothing (it names no items); a collected portion returns nothing (the goods are gone). ⚠ Inventing stock is worse than not returning it.
- [X] T079 [P] ✅ All four cases, container-backed; the movement cites the order. Test (**US1**) Test (**US1**): the three cases above, and that a `refund` movement is written with the order cited.
- [X] T080 [P] ⚠ **THE SAME DEFECT SHAPE, A THIRD TIME.** 053 fixed the VALUE and kept the SHAPE — a negation against one name still cannot notice the vocabulary moving, and it moved again. Both new terminal states satisfied `<> 'delivered'` and would have held a customer for **seven days** over a package nobody is carrying. Terminal states are now named **positively**. **NEGATIVE PROOF ✅ EXECUTED** (2 tests). FR-032 FR-032: a cancelled or fully refunded order must not block account closure as "in transit" — the 053 fix, extended to the states this slice introduces.
- [X] T081 [P] Four counters, ⚠ **each with its label name pinned per metric**. Metrics Four counters in `internal/platform/metrics/metrics.go` ([research.md](./research.md) R9). ⚠ Pin the **label name per metric** — 054 declared one with `outcome` and called it with `stage`, which does not panic; it silently emits a series every alert misses.
- [X] T082 [P] ⚠ **AND IT CAUGHT A MISSING CALL SITE** — `RefundIssued` was declared and never incremented on the success path, which reads as zero on a dashboard, indistinguishable from "this never happens". **NEGATIVE PROOFS ✅ EXECUTED** (label mismatch; an `order_id` label). Test Test: every counter has a call site, and no label carries an order id, customer id or amount.
- [X] T083 Written and **INERT**, registered in the README. Three rules — a sustained failure rate, refunds that never settle (⚠ the silent one: an insufficient-balance hold looks like nothing at all), and ambiguous submissions. The file carries the SQL to run until a stack exists. ⚠ `infra/observability/alerts/055-refunds-cancellation.yml` + register it in that README — a sustained `failed` rate, and a refund stuck in `submitted` (the shape an insufficient-balance hold takes, which otherwise looks like nothing at all). ⚠ **Written and INERT**: no Prometheus stack exists. Recorded in the plan's Complexity Tracking.
- [X] T084 [P] **NEGATIVE PROOFS ✅ EXECUTED** (an `UPDATE` touching `amount`; a `DELETE`). ⚠ It also asserts it is **actually scanning source** — a source guard that finds no files always passes. Append-only guard Append-only guard for `public.refund` — no `UPDATE`/`DELETE` outside the permitted status transitions. 054's mechanism, second outing.
- [X] T085 [P] `typecheck` **19/19** · `pnpm -r test` **1,813** across **17 reporting packages** · Go clean · refunds **67** · edge-orders **54** · edge-shop **192** · edge-customer **203** · `email-check` **12**. ⚠ Reporting packages counted, per 029. Run every machine gate in [quickstart.md](./quickstart.md) §1 and record the counts. ⚠ Count reporting packages — 029 found `pnpm -r test` green while `typecheck` failed.
- [X] T086 [P] ✅ Confirmed unchanged: `saveditems` (`public.delivery_pricing_rule does not exist`) and `platform/delivery` (`sameday_eligible does not exist`). `check-no-phantm` still fails on 042/045/050 prose — also pre-existing. Confirm the two pre-existing red gates are unchanged at clean HEAD (`saveditems`, `platform/delivery`) so they are not attributed to this slice.
- [X] T087 [P] All three, incl. a full **Refunds and cancellation** section in the operator guide with the state table and the ⚠ *"no answer from the bank"* warning. Update `docs/audiences/customer-capabilities.md` §055, `docs/audiences/shop-capabilities.md` §055, and ⚠ **`docs/order-console-guide.md`** — 053's operator guide for the very screen this slice adds a money-moving control to.
- [X] T088 [P] **G3 CLOSED**, with what it does **not** close recorded: replacements, disputes, substitutions, returns, shop cost attribution. Update `ORDER-FLOW-GAPS.md` — **G3 closed**; ⚠ record what it does NOT close: replacements, disputes, substitutions, and shop cost attribution.
- [X] T089 [P] Update `CLAUDE.md`'s Active feature section. Update `CLAUDE.md`'s Active feature section.
- [X] T090 Write `specs/055-refunds-cancellation/SIGNOFF.md`. Write `specs/055-refunds-cancellation/SIGNOFF.md` — what was machine-verified, what a person walked, and what was not.
